  ...
  Widget build(BuildContext context) {
    ListTile makeListTile(project.Tools tool) {
      ...
      for (int i = 0; i < tool.stock.length; i++) {
        print(
            "--- tool stock entry quantity is ${tool.stock[i].quantity.trim()}");
        if (tool.stock[i].quantity.trim().contains("-")) {
          Overseer.activeToolQuantityAndUnit =
              Overseer.activeToolQuantityAndUnit +
                  "(Issued: " +
                  (tool.stock[i].quantity).trim().substring(1) +
                  "" +
                  tool.unitTitle +
                  ")";
        } else {
      ...
      return ListTile(
        ...
        trailing: Icon(Icons.keyboard_arrow_right, color: Color(0xff5E70D8), size: 30.0),
        onTap: () {
          Overseer.activeToolQuantityAndUnit = "";
          for (int i = 0; i < tool.stock.length; i++) {
            if (tool.stock[i].quantity.trim().contains("-")) {
    ...
    final makeBody = Container(
      ...
      child: ListView.builder(
        ...
        itemCount: Overseer.isMaterialToolRefreshed
            ? Overseer.myProjectToolsList.length
            : Overseer.myToolList.length,
        itemBuilder: (BuildContext context, int index) {
          if (Overseer.isMaterialToolRefreshed) {
            ...
            for (int i = 0; i < Overseer.myToolList.length; i++) {
              ...
              print(
                  "AA --- stock length ${Overseer.myToolList[i].stock.length}");
              for (int y = 0; y < Overseer.myToolList[i].stock.length; y++) {
                print(
                    "AA ***** Stock entry ${Overseer.myToolList[i].stock[y].quantity} ");
              }
            }
            print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
            ...
            for (int i = 0; i < Overseer.myProjectToolsList.length; i++) {
              ...
              print(
                  "BB --- stock length ${Overseer.myProjectToolsList[i].stock.length}");
              for (int y = 0;
                  y < Overseer.myProjectToolsList[i].stock.length;
                  y++) {
                print(
                    "BB ***** Stock entry ${Overseer.myProjectToolsList[i].stock[y].quantity} ");
              }
            }
            ...
            Overseer.myProjectToolsList[index] =
                Overseer.myToolList[index] as project.Tools;
            return makeCard(Overseer.myProjectToolsList[index]);
          }
        },
      ),
